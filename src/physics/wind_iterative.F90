    ...
    subroutine calc_iter_winds(domain, alpha_in, div_in, adv_den)
        ...
        if (operator_probed .and. wind_hypre_available()) then
            ...
            if (status == 0) then
                ...
                if (STD_OUT_PE) then
                    write(*,'(A,I0,A,ES12.4,A,ES12.4)') ' HICAR HYPRE FGMRES+MGR/AMG: iterations=', n_iters, &
                        ' true_residual=', res_final, ' target=', target_norm
                    flush(output_unit)
                endif
                if (res_final > target_norm) then
                    if (STD_OUT_PE) write(*,'(A,ES12.4,A,ES12.4,A,ES12.4)') &
                        ' HICAR HYPRE FGMRES+MGR/AMG rejected by true residual: ', res_final, ' target=', target_norm, &
                        ' max |x|=', max_x_global
                    status = 3
                endif
            else if (STD_OUT_PE) then
                write(*,'(A,I0,A,I0)') ' HICAR HYPRE FGMRES+MGR/AMG failed: status=', status, &
                    ' iterations=', n_iters
                flush(output_unit)
            endif
        else
