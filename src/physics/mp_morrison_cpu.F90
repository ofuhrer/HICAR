...
! currently mixing of number concentrations also is neglected (not coupled with PBL schemes)
   ...
   !$omp private(PRECPRT1D,SNOWRT1D,SNOWPRT1D,GRPLPRT1D,ISED,SSED,C2PREC,CSED,GSED,RSED) &
   !$omp private(QGSTEN,QRSTEN, QISTEN, QNISTEN, QCSTEN, EFFC1D, EFFI1D, EFFS1D, EFFR1D, EFFG1D) &
...
! TOP OF MODEL
      ...
      K = KTE
! Reciprocal-multiply form, kept bit-identical with mp_morrison_gpu (see
! tests: compiling the GPU module into a host build must reproduce this exactly)
      DUM1 = 1.0/DZQ(K)
      FALTNDR = FALOUTR(K)*DUM1
      FALTNDI = FALOUTI(K)*DUM1
      FALTNDNI = FALOUTNI(K)*DUM1
      FALTNDS = FALOUTS(K)*DUM1
      FALTNDNS = FALOUTNS(K)*DUM1
      FALTNDNR = FALOUTNR(K)*DUM1
      FALTNDC = FALOUTC(K)*DUM1
      FALTNDNC = FALOUTNC(K)*DUM1
      FALTNDG = FALOUTG(K)*DUM1
      FALTNDNG = FALOUTNG(K)*DUM1
! ADD FALLOUT TERMS TO EULERIAN TENDENCIES

      DUM2 = 1.0/NSTEP/RHO(K)
      QRSTEN(K) = QRSTEN(K)-FALTNDR*DUM2
      QISTEN(K) = QISTEN(K)-FALTNDI*DUM2
      NI3DTEN(K) = NI3DTEN(K)-FALTNDNI*DUM2
      QNISTEN(K) = QNISTEN(K)-FALTNDS*DUM2
      NS3DTEN(K) = NS3DTEN(K)-FALTNDNS*DUM2
      NR3DTEN(K) = NR3DTEN(K)-FALTNDNR*DUM2
      QCSTEN(K) = QCSTEN(K)-FALTNDC*DUM2
      NC3DTEN(K) = NC3DTEN(K)-FALTNDNC*DUM2
      QGSTEN(K) = QGSTEN(K)-FALTNDG*DUM2
      NG3DTEN(K) = NG3DTEN(K)-FALTNDNG*DUM2

      DUMT = DT/NSTEP
      DUMR(K) = DUMR(K)-FALTNDR*DUMT
      DUMI(K) = DUMI(K)-FALTNDI*DUMT
      DUMFNI(K) = DUMFNI(K)-FALTNDNI*DUMT
      DUMQS(K) = DUMQS(K)-FALTNDS*DUMT
      DUMFNS(K) = DUMFNS(K)-FALTNDNS*DUMT
      DUMFNR(K) = DUMFNR(K)-FALTNDNR*DUMT
      DUMC(K) = DUMC(K)-FALTNDC*DUMT
      DUMFNC(K) = DUMFNC(K)-FALTNDNC*DUMT
      DUMG(K) = DUMG(K)-FALTNDG*DUMT
      DUMFNG(K) = DUMFNG(K)-FALTNDNG*DUMT

      DO K = KTE-1,KTS,-1
      DUM1 = 1.0/DZQ(K)
      FALTNDR = (FALOUTR(K+1)-FALOUTR(K))*DUM1
      FALTNDI = (FALOUTI(K+1)-FALOUTI(K))*DUM1
      FALTNDNI = (FALOUTNI(K+1)-FALOUTNI(K))*DUM1
      FALTNDS = (FALOUTS(K+1)-FALOUTS(K))*DUM1
      FALTNDNS = (FALOUTNS(K+1)-FALOUTNS(K))*DUM1
      FALTNDNR = (FALOUTNR(K+1)-FALOUTNR(K))*DUM1
      FALTNDC = (FALOUTC(K+1)-FALOUTC(K))*DUM1
      FALTNDNC = (FALOUTNC(K+1)-FALOUTNC(K))*DUM1
      FALTNDG = (FALOUTG(K+1)-FALOUTG(K))*DUM1
      FALTNDNG = (FALOUTNG(K+1)-FALOUTNG(K))*DUM1
